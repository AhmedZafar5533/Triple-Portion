...
import {
    Box, ShoppingBasket, Scale, Calendar, AlertCircle, Power, Loader2, Search, Plus, Edit2, Trash2
} from 'lucide-react';
import { toast } from 'sonner';
import { motion, AnimatePresence } from 'framer-motion';
import { IMG_BASE_URL } from '../../config';

const GroceryList = ({ onEdit, onAdd }) => {
    ...
    const groceries = myProducts.filter(p =>
        p.category === 'Grocery' &&
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-gray-800 p-8 rounded-[3rem] border border-gray-100 dark:border-gray-700 shadow-xl shadow-blue-500/5 transition-all">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Grocery Inventory</h1>
                    <p className="text-sm text-gray-500 font-bold uppercase tracking-widest mt-1">{groceries.length} Products Tracked</p>
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-[3rem] border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-gray-50 dark:border-gray-700/50">
                                ...
                                <th className="px-6 py-6 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Weight / Volume</th>
                                ...
                                <th className="px-6 py-6 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Expiry Status</th>
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                                                ...
                                                <span className="text-xs font-bold">{item.weight} {item.unit}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className="flex flex-col gap-1">
                                                <span className={`text-xs font-bold ${item.stock < 10 ? 'text-red-500' : 'text-gray-900 dark:text-white'}`}>
                                                    {item.stock} in stock
                                                </span>
                                                {item.stock < 10 && (
                                                    <span className="text-[9px] font-bold text-red-400 uppercase tracking-tighter">Low Inventory</span>
                                                )}
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            ...
                                            {item.expiryDate ? (
                                                <div className="flex flex-col gap-1">
                                                    <div className="flex items-center gap-1.5 text-gray-500">
                                                        <Calendar size={12} />
                                                        <span className="text-[10px] font-bold">{new Date(item.expiryDate).toLocaleDateString()}</span>
                                                    </div>
                                                ...
                                                <span className="text-[10px] font-bold text-gray-400 italic">No Expiry Set</span>
                                            )}
                                        </td>
                ...
                {!loading && groceries.length === 0 && (
                    <div className="p-20 text-center">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mb-8 font-medium">Add your first grocery product to start selling.</p>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-white dark:bg-gray-800 w-full max-w-md rounded-[3rem] p-10 shadow-2xl space-y-6">
                            <div className="text-center space-y-4">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Remove Product?</h3>

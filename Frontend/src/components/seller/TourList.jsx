...
import {
    AlertTriangle, Box, MapPin, Clock, Users, Calendar, Power, Loader2, Search, Plus, Edit2, Trash2
} from 'lucide-react';
import { toast } from 'sonner';
import { motion, AnimatePresence } from 'framer-motion';
import { IMG_BASE_URL } from '../../config';

const TourList = ({ onEdit, onAdd }) => {
    ...
    const tours = myProducts.filter(p =>
        p.category === 'Tour' &&
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-gray-800 p-8 rounded-[3rem] border border-gray-100 dark:border-gray-700 shadow-xl shadow-blue-500/5 transition-all">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Travel Experiences</h1>
                    <p className="text-sm text-gray-500 font-bold uppercase tracking-widest mt-1">{tours.length} Active Packages</p>
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-[3rem] border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-gray-50 dark:border-gray-700/50">
                                <th className="px-8 py-6 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Experience Info</th>
                                <th className="px-6 py-6 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Duration & Difficulty</th>
                                <th className="px-6 py-6 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Group Size</th>
                                    ...
                                    >
                                        <td className="px-8 py-5">
                                            <div className="flex items-center gap-4">
                                                <div className="h-14 w-14 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-700 bg-gray-50 flex-shrink-0">
                                                    <img
                                                        src={tour.images && tour.images[0] ? `${IMG_BASE_URL}${tour.images[0]}` : null}
                                                        className="h-full w-full object-cover"
                                                        alt=""
                                                    />
                                                </div>
                                                <div>
                                                    <p className="text-sm font-bold text-gray-900 dark:text-white line-clamp-1">{tour.name}</p>
                                                    <div className="flex items-center gap-1 text-blue-600 mt-0.5">
                                                        <MapPin size={10} />
                                                        <p className="text-[10px] font-bold uppercase tracking-widest">{tour.location}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className="space-y-1">
                                                ...
                                                <span className="inline-block px-2 py-0.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 rounded-md text-[9px] font-bold uppercase tracking-widest">
                                                    {tour.difficulty}
                                                </span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                                                <Users size={14} />
                                                <span className="text-xs font-bold">Max {tour.maxGroupSize}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <p className="text-sm font-bold text-blue-600">UGX {tour.price.toLocaleString()}</p>
                                            <p className="text-[9px] font-bold text-gray-400 uppercase tracking-tighter">Per Person</p>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className="flex flex-col gap-1.5">
                                                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${tour.availabilityStatus === 'Available'
                                                        ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20'
                                                        : 'bg-gray-100 text-gray-500 dark:bg-gray-700/50 dark:text-gray-400'
                                                    }`}>
                                                    <div className={`w-1.5 h-1.5 rounded-full ${tour.availabilityStatus === 'Available' ? 'bg-emerald-500' : 'bg-gray-400'}`} />
                                                    {tour.availabilityStatus}
                                                </span>
                                                {tour.availabilityStatus === 'Unavailable' && tour.disableReason && (
                                                    <p className="text-[8px] font-bold text-gray-400 max-w-[120px] italic">Reason: {tour.disableReason}</p>
                                                )}
                                                {tour.adminDisabled && (
                                            ...
                                            <div className="flex items-center justify-end gap-2 transition-all text-gray-400">
                                                <button
                                                    onClick={() => setStatusModal({
                                                        product: tour,
                                                        action: tour.availabilityStatus === 'Available' ? 'disable' : 'enable'
                                                    })}
                                                    className={`p-2.5 rounded-xl transition-all ${tour.availabilityStatus === 'Available'
                                                    ...
                                                    title={tour.availabilityStatus === 'Available' ? 'Disable Listing' : 'Enable Listing'}
                                                >
                                                    <Power size={18} />
                                                </button>
                                                <button
                ...
                {!loading && tours.length === 0 && (
                    <div className="p-20 text-center">
                        ...
                        <h3 className="text-xl font-bold dark:text-white mb-2">No experiences listed</h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mb-8 font-medium">Your travel packages will appear here once listed.</p>
                        <button onClick={onAdd} className="text-blue-600 font-bold uppercase tracking-widest text-[10px] hover:tracking-[0.2em] transition-all">Create First Package</button>
                    </div>
                )}
            </div>
                            ...
                            <div className="text-center space-y-4">
                                ...
                                <div className="space-y-2">
                                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white uppercase tracking-tight">
                                        {statusModal.action === 'disable' ? 'Disable Experience?' : 'Re-enable Experience?'}
                                    </h2>
                                    <p className="text-sm text-gray-500 font-medium italic px-4">
                                        {statusModal.action === 'disable'
                                            ? "Temporarily take this experience off the market."
                                            : "Put this experience back on the market for customers."}
                                    </p>
                                </div>
                            </div>
                            ...
                            {statusModal.action === 'disable' && (
                                <div className="space-y-2">
                                    ...
                                    <textarea
                                        ...
                                        placeholder="e.g., Seasonal break, Guide unavailable, Maintenance..."
                                    />
                                </div>
                            )}
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-white dark:bg-gray-800 w-full max-w-md rounded-[3rem] p-10 shadow-2xl space-y-6">
                            <div className="text-center space-y-4">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Delete Experience?</h3>
                                    <p className="text-sm text-gray-500 font-medium">Are you sure you want to remove <span className="font-bold text-gray-900 dark:text-white">"{deleteConfirm.name}"</span>? All itenary data and images will be lost.</p>
                                </div>
                            </div>
                            <div className="flex gap-3">
